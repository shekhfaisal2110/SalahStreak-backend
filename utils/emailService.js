...
export const sendEmail = async ({ to, subject, htmlContent }) => {
  ...
  const sendSmtpEmail = {
    to: [{ email: to }],
    sender: { email: process.env.EMAIL_FROM, name: 'Salah Streak' },
    subject,
    htmlContent,
  };
    ...
    <body style="margin:0; padding:0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color:#f4f4f5; line-height:1.5;">
      <table width="100%" cellpadding="0" cellspacing="0" border="0" style="background-color:#f4f4f5; padding:20px;">
        <tr>
          <td align="center">
            <table width="100%" max-width="500px" cellpadding="0" cellspacing="0" border="0" style="max-width:500px; width:100%; background-color:#ffffff; border-radius:32px; box-shadow:0 10px 25px rgba(0,0,0,0.05); overflow:hidden;">
              <tr><td style="background: linear-gradient(135deg, #10b981 0%, #059669 100%); padding:32px 24px; text-align:center;"><h1 style="margin:0; color:#ffffff; font-size:28px;">🕌 Salah Streak</h1><p style="margin:8px 0 0; color:#e6f7f0;">Your spiritual companion</p></td></tr>
              <tr><td style="padding:32px 24px;">
                <h2 style="margin:0 0 8px; color:#1e293b;">${subject}</h2>
                <p style="margin:0 0 24px; color:#475569;">We received a request to ${action}. Use the code below. Valid for <strong>10 minutes</strong>.</p>
                <div style="background-color:#f1f5f9; border-radius:24px; padding:24px; text-align:center; margin-bottom:24px;"><span style="font-size:40px; font-weight:800; letter-spacing:8px; color:#0f172a;">${otp}</span></div>
                <p style="margin:0 0 16px; color:#475569;">If you didn't request this, ignore this email.</p>
                <hr style="border:0; border-top:1px solid #e2e8f0; margin:24px 0;" />
                <p style="margin:0; color:#94a3b8; text-align:center;">May your day be filled with blessings.</p>
              </td></tr>
              <tr><td style="background-color:#f8fafc; padding:20px 24px; text-align:center;"><p style="margin:0; color:#64748b;">© ${new Date().getFullYear()} Salah Streak</p></td></tr>
            </table>
          </td>
        </tr>
      </table>
...
// Registration Email (includes OTP and login key)
export const sendRegistrationEmail = async (email, name, otp, loginKey) => {
  const subject = 'Welcome to Salah Streak – Verify Your Account';
  const htmlContent = `
    <!DOCTYPE html>
    <html>
    <head><meta charset="UTF-8"><title>Welcome to Salah Streak</title></head>
    <body style="margin:0; padding:0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color:#f4f4f5;">
      <table width="100%" cellpadding="0" cellspacing="0" border="0" style="background-color:#f4f4f5; padding:20px;">
        <tr><td align="center">
          <table width="100%" max-width="500px" style="max-width:500px; background:#ffffff; border-radius:32px; overflow:hidden;">
            <tr><td style="background: linear-gradient(135deg, #10b981 0%, #059669 100%); padding:32px 24px; text-align:center;"><h1 style="color:#fff;">🕌 Salah Streak</h1><p style="color:#e6f7f0;">Your spiritual companion</p></td></tr>
            <tr><td style="padding:32px 24px;">
              <h2>Assalamu Alaikum ${name}!</h2>
              <p>Thank you for registering. Your OTP is:</p>
              <div style="background:#f1f5f9; border-radius:24px; padding:24px; text-align:center; margin:20px 0;"><span style="font-size:40px; font-weight:800; letter-spacing:8px;">${otp}</span></div>
              <p>Your unique login key (keep it safe):</p>
              <div style="background:#e0f2fe; border-radius:16px; padding:16px; text-align:center; margin:20px 0;"><span style="font-size:24px; font-weight:700; color:#0369a1;">${loginKey}</span></div>
              <p>You can use this key to log in instead of email and password.</p>
              <p>The OTP expires in 10 minutes.</p>
            </td></tr>
            <tr><td style="background:#f8fafc; padding:20px; text-align:center;"><p>© ${new Date().getFullYear()} Salah Streak</p></td></tr>
          </table>
        </td></tr>
      </table>
    </body>
...
// Optional: Missed Prayer Reminder (you can keep or remove)
export const sendMissedPrayerNotification = async (email, name, missedPrayers) => {
  const subject = 'Missed Prayer Reminder';
  const prayersList = missedPrayers.join(', ');
  const htmlContent = `<h2>Assalamu Alaikum ${name},</h2><p>You missed: ${prayersList}. Make them up.</p>`;
  await sendEmail({ to: email, subject, htmlContent });
};

// Optional: Streak Milestone
export const sendStreakMilestoneEmail = async (email, name, streak) => {
  const subject = `🎉 Mubarak! ${streak}-day streak!`;
  const htmlContent = `<h2>Masha'Allah! You've reached ${streak} days.</h2>`;
  await sendEmail({ to: email, subject, htmlContent });
};

// Optional: Prayer Reminder
export const sendPrayerReminderEmail = async (email, name, streak) => {
  const subject = '⏰ Don’t break your streak!';
  const htmlContent = `<h2>Assalamu Alaikum ${name},</h2><p>You have a ${streak}-day streak. Log your prayers now!</p>`;
  await sendEmail({ to: email, subject, htmlContent });
};