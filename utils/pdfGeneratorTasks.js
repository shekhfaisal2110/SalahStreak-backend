...
export const generateTaskReportPDF = (data, startDate, endDate) => {
  ...
  return new Promise((resolve) => {
    ...
    // Header
    doc.fontSize(24).fillColor('#065f46').text('Completed Tasks Report', { align: 'center' });
    doc.moveDown(0.5);
    ...
    doc.text(`Total Tasks Completed: ${data.length}`);
    doc.moveDown(2);
    ...
    const headers = ['Task Name', 'Completed At'];
    const rowHeight = 25;
    let y = doc.y;
    ...
    if (data.length === 0) {
      ...
      data.forEach((item, idx) => {
        ...
        doc.text(item.taskName, x + 5, y + 5, { width: colWidths[0], align: 'center' });
        x += colWidths[0];
        doc.text(item.completedAt, x + 5, y + 5, { width: colWidths[1], align: 'center' });
