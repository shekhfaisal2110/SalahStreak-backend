  ...
  try {
    ...
    const [tasks, totalCount] = await Promise.all([
      Task.find({ user: req.user._id })
        ...
        .skip(skip)
        .limit(limit)
        .lean(), // plain JS objects – 10x faster
      Task.countDocuments({ user: req.user._id })
    ]);
...
export const deleteTask = async (req, res) => {
  try {
    ...
    const task = await Task.findOneAndDelete({ _id: id, user: req.user._id });
    if (!task) return res.status(404).json({ success: false, message: 'Task not found' });
    // Delete all entries for this task asynchronously (non‑blocking)
    TaskEntry.deleteMany({ task: id }).catch(err => console.error('Failed to delete entries:', err));
    res.json({ success: true, message: 'Task deleted' });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
...
export const getEntriesForMonth = async (req, res) => {
  try {
    ...
    const entries = await TaskEntry.find({
      ...
      .sort({ date: 1 })
      .limit(100) // max 31 days anyway, safe
      .lean();
...
// Toggle a task entry for a specific date (only today allowed)
export const toggleEntry = async (req, res) => {
  try {
    const { taskId, date } = req.params;
    const userId = req.user._id;
    ...
    if (date !== today) {
      return res.status(400).json({ 
        success: false, 
        message: 'You can only log today\'s task.' 
      });
    }

    // Find existing entry or create a new one with completed: false
    let entry = await TaskEntry.findOne({ user: userId, task: taskId, date });
    if (!entry) {
      entry = new TaskEntry({ user: userId, task: taskId, date, completed: true });
    } else {
      entry.completed = !entry.completed;
    }
    await entry.save();

    // (Optional) Update task stats like streak – you may already have separate logic.
    // For now just return the updated entry.
    res.json({ success: true, data: entry });
  } catch (error) {
...
// Optimized streak calculation (uses lean, limits to 1000 entries)
export const getTaskStats = async (req, res) => {
  try {
    const { taskId } = req.params;
    const userId = req.user._id;
    ...
    res.json({ success: true, data: { streak, totalCompleted: completions.length } });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
// Get all entries for a task for a full year (lean, limit 366)
export const getEntriesForYear = async (req, res) => {
  try {
    const { taskId } = req.params;
    const { year } = req.query;
