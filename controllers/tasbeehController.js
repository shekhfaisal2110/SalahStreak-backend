...
// ========== GET all tasbeeh for user (paginated) ==========
export const getTasbeehList = async (req, res) => {
  try {
    const page = parseInt(req.query.page) || 1;
    const limit = parseInt(req.query.limit) || 50;
    ...
    console.error('getTasbeehList error:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};
...
export const createTasbeeh = async (req, res) => {
  try {
    ...
    // Case‑insensitive search for an existing tasbeeh with the same name
    const existing = await Tasbeeh.findOne({
      user: userId,
      name: { $regex: new RegExp(`^${name}$`, 'i') }
    }).lean();

    if (existing) {
      // Increment the existing tasbeeh's count by 1
      const updated = await Tasbeeh.findOneAndUpdate(
        { _id: existing._id, user: userId },
        {
          $inc: { currentCount: 1 },
          $set: { targetCount: targetCount || existing.targetCount } // optionally update target
        },
        { new: true, lean: true }
      );
      return res.status(200).json({
        success: true,
        message: 'Tasbeeh already exists – count increased by 1',
        data: updated
      });
    }

    // No duplicate – create a brand new tasbeeh
    const newTasbeeh = await Tasbeeh.create({
      user: userId,
      name,
      arabicName,
      targetCount: targetCount || 33,
    ...
    res.status(201).json({ success: true, data: newTasbeeh });
  } catch (error) {
    console.error('createTasbeeh error:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};

// ========== INCREMENT tasbeeh (atomic, with daily update) ==========
export const incrementTasbeeh = async (req, res) => {
  try {
    const { id } = req.params;
    let { count = 1 } = req.body;
    ...
    // Step 1: Increment currentCount atomically (only if not completed)
    const updated = await Tasbeeh.findOneAndUpdate(
      { _id: id, user: req.user._id, completed: false },
      { $inc: { currentCount: count } },
      { returnDocument: 'after', lean: true }   // ✅ fixed deprecated 'new'
    );
    ...
    // Step 2: If target reached, mark as completed (atomic)
    let finalTasbeeh = updated;
    if (updated.currentCount >= updated.targetCount && !updated.completed) {
      finalTasbeeh = await Tasbeeh.findOneAndUpdate(
        { _id: id, user: req.user._id },
        { $set: { completed: true, completedAt: new Date() } },
    ...
    // Step 3: Update daily count in background (fire-and-forget)
    TasbeehDaily.findOneAndUpdate(
      { user: req.user._id, tasbeeh: id, date: today },
      { $inc: { count } },
      { upsert: true, lean: true }
    ...
    res.json({ success: true, data: finalTasbeeh });
  } catch (error) {
    console.error('Increment error:', error);
    res.status(500).json({ success: false, message: error.message });
  }
...
export const resetTasbeeh = async (req, res) => {
  try {
    ...
    console.error('resetTasbeeh error:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};

// ========== DELETE tasbeeh (and its daily records in background) ==========
export const deleteTasbeeh = async (req, res) => {
  try {
    const result = await Tasbeeh.findOneAndDelete({ _id: req.params.id, user: req.user._id });
    if (!result) return res.status(404).json({ success: false, message: 'Tasbeeh not found' });
    ...
    console.error('deleteTasbeeh error:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};

// ========== UPDATE target count (with automatic completion check) ==========
export const updateTarget = async (req, res) => {
  try {
    const { id } = req.params;
    let { targetCount } = req.body;
    ...
    const newCompleted = tasbeeh.currentCount >= targetCount;
    const update = {
      targetCount,
...
// ========== GET daily totals for last N days (aggregation) ==========
export const getDailyTotals = async (req, res) => {
  try {
    const { days = 30 } = req.query;
...
export const togglePin = async (req, res) => {
  try {
    ...
    const updated = await Tasbeeh.findOneAndUpdate(
      ...
      { returnDocument: 'after', lean: true, fields: { pinned: 1, name: 1 } }
    );
    res.json({ success: true, data: updated });
  } catch (error) {
...
export const toggleShowCount = async (req, res) => {
  try {
    ...
    const updated = await Tasbeeh.findOneAndUpdate(
      { _id: id, user: req.user._id },
      { $set: { showCount: !tasbeeh.showCount } },
      { returnDocument: 'after', lean: true, fields: { showCount: 1, name: 1 } }
    );
    res.json({ success: true, data: updated });
  } catch (error) {
    console.error('toggleShowCount error:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};

// ========== GET daily totals by date range (aggregation) ==========
export const getDailyTotalsByDateRange = async (req, res) => {
  try {
    let { startDate, endDate } = req.query;
    if (!startDate || !endDate) {
    ...
    console.error('getDailyTotalsByDateRange error:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};

// ========== GET daily completion for a single tasbeeh (calendar, max 90 days) ==========
export const getTasbeehDailyCompletion = async (req, res) => {
  try {
    const { id } = req.params;
    let { startDate, endDate } = req.query;
    ...
    const [tasbeeh, dailyRecords] = await Promise.all([
      Tasbeeh.findOne({ _id: id, user: req.user._id }).lean(),
      TasbeehDaily.find({
        user: req.user._id,
        tasbeeh: id,
        date: { $gte: startDate, $lte: endDate }
    ...
    console.error('getTasbeehDailyCompletion error:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};