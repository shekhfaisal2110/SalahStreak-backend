...
// Get or create current active progress for user (most recent incomplete)
export const getCurrentProgress = async (req, res) => {
  try {
    const userId = req.user._id;
    let progress = await QuranProgress.findOne({ user: userId, completed: false })
      .sort({ startDate: -1 })
      .lean();
...
// Start a new progress (if no active one exists)
export const startNewProgress = async (req, res) => {
  try {
    const userId = req.user._id;
    // Check for existing active progress
    const existing = await QuranProgress.findOne({ user: userId, completed: false }).lean();
    if (existing) {
      return res.status(400).json({ success: false, message: 'You already have an active progress. Complete or reset it first.' });
    }
    ...
    res.status(201).json({ success: true, data: progress.toObject() });
  } catch (error) {
    console.error(error);
    res.status(500).json({ success: false, message: error.message });
  }
...
// Mark a specific para (1‑based para number from route param)
export const markPara = async (req, res) => {
  try {
    const { para } = req.params; // para number 1‑30
    const paraIndex = parseInt(para) - 1;
    if (paraIndex < 0 || paraIndex > 29) {
      return res.status(400).json({ success: false, message: 'Para number must be between 1 and 30' });
    }
    ...
    // Toggle or set to true? Usually marking means set to true.
    progress.paras[paraIndex] = true;

    // Check if all paras are now completed
    const allCompleted = progress.paras.every(p => p === true);
    if (allCompleted) {
      progress.completed = true;
      progress.completedAt = new Date();
      ...
      await QuranCompletion.create({
        user: userId,
        completedAt: progress.completedAt,
        durationDays: Math.ceil((progress.completedAt - progress.startDate) / (1000 * 60 * 60 * 24))
      });
    }
    await progress.save();

    const updatedProgress = await QuranProgress.findById(progress._id).lean();
...
// Get completions by date range (for reports)
export const getCompletionsByDateRange = async (req, res) => {
  try {
    let { startDate, endDate } = req.query;
    const query = { user: req.user._id };
...
export const togglePara = async (req, res) => {
  try {
    const { para } = req.params;
    const paraIndex = parseInt(para) - 1;
    if (paraIndex < 0 || paraIndex > 29) {
    ...
    let progress = await QuranProgress.findOne({ user: userId, completed: false });
    if (!progress) {
      return res.status(404).json({ success: false, message: 'No active progress found' });
    }

    // Toggle the para status
    const newStatus = !progress.paras[paraIndex];
    progress.paras[paraIndex] = newStatus;

    // If unmarking, ensure completed flag is false
    if (!newStatus && progress.completed) {
    ...
    if (allCompleted) {
      progress.completed = true;
      progress.completedAt = new Date();
      // Create completion record (avoid duplicate)
      const existingCompletion = await QuranCompletion.findOne({ user: userId, completedAt: progress.completedAt });
      ...
      // If we unmarked a para and it was previously completed, remove completion status
      progress.completed = false;
      progress.completedAt = null;
    }
    ...
    console.error('Toggle para error:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};